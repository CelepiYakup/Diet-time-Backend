...
export class UserController {
  ...
  static async registerUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      const existingUserByUsername = await UserModel.getUserByUsername(userData.username);
  ...
  static async getUserById(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (req.user && req.user.id !== userId) {
        res.status(403).json({ message: 'Access denied. You can only view your own account.' });
        return;
      }
  ...
  static async updateUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (req.user && req.user.id !== userId) {
        res.status(403).json({ message: 'Access denied. You can only update your own account.' });
        return;
      }
